...
resource "azurerm_key_vault_access_policy" "stamp_appservice" {
  ...
  key_vault_id = azurerm_key_vault.stamp[each.key].id
  tenant_id    = data.azurerm_client_config.current.tenant_id
  object_id    = azurerm_user_assigned_identity.loganalytics_reader_mi[each.key].principal_id #azurerm_linux_web_app.appservice[each.key].identity[0].principal_id
